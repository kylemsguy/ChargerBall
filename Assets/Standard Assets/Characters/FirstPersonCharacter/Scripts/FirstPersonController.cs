...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private AudioSource m_ChargeAudioSource;
        ...
        private void Start()
        {
            ...
            m_ChargeAudioSource = audioSources[1];
            m_MouseLook.Init(transform, m_Camera.transform);
            m_PitchDelta = m_GunChargeSpeed / 20.0f;
        }
        ...
        private void Update()
        {
            ...
            if (!m_isChargingGun && CrossPlatformInputManager.GetButtonDown("Fire1"))
            {
                m_isChargingGun = true;
            }
            else if (m_isChargingGun && CrossPlatformInputManager.GetButtonUp("Fire1"))
            {
                m_isChargingGun = false;
            }
        ...
        private void StartChargeSoundEffect()
        {
            m_ChargeAudioSource.pitch = startingPitch;
            m_ChargeAudioSource.Play();
        }

        private void UpdateChargeSoundEffect()
        {
            // max charge value is around 6
            m_ChargeAudioSource.pitch += m_PitchDelta;
        }

        private void StopChargeSoundEffect()
        {
        ...
        private void FixedUpdate()
        {
            ...
            if (m_isChargingGun)
            {
                if (m_GunChargeLevel == 0)
                {
                    ...
                    StartChargeSoundEffect();
                    m_GunChargeLevel += m_GunChargeSpeed;
                }
                else if (m_GunChargeLevel < 100)
                {
                    // still charging
                    UpdateChargeSoundEffect();
                    m_GunChargeLevel += m_GunChargeSpeed;
                }
                else
                {
                    StopChargeSoundEffect();
                    m_GunChargeLevel = 100;
                    // Fully charged! Ready to shoot at full power.
                }
                //m_IsFiring = false;
            }
            else
